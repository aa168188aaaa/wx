...
// URL 参数
var urlParams = {
    keyID: Infoc.queryString('keyID') || '1',
    ...
    msOcpcId: Infoc.queryString('msclkidduba') || '' // 原是msclkid 暂下
};

// 页面信息
var pathnameArr = window.location.pathname.split('/');
var pageName = pathnameArr[pathnameArr.length-1].split('.')[0].substring(1);
...
var fanXingHandler = {
    init: function() {
        ...
        try {
            ...
            this.updateDownloadLinks(defaultLink, channelLink);
        }
    },
    judgeOcpc: function() {
        if (ocpcTypeNum !== 0) {
    ...
    setKeyID: function () {
        var originalDownloadUrl = jDownload.attr('href')
        originalDownloadUrl = utils.checkDownUrl(originalDownloadUrl, keyIDStr)
    
        jDownload.attr('href', originalDownloadUrl)
    },
    ...
    updateDownloadLinks: function(mainUrl, channelUrl) {
        ...
        if (urlParams.keyID !== '' && (urlParams.sfrom == '166' || urlParams.sfrom == '206' || urlParams.sfrom == '216')) {
            this.setKeyID()
        }
